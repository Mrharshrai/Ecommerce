...
public class UserController {
    ...
    public ResponseEntity<String> deleteAccount(@PathVariable("id") Long id, @RequestBody @Valid String reason, Authentication authentication, HttpServletRequest httpRequest) {
        ...
        if (currentUser.getRoles().contains(Role.ADMIN)) {
            // FIX: Use currentUser.getEmail() or currentUser.getName() instead of passing the whole object
            description = String.format("Admin (%s) permanently deleted customer account (Target ID: %d). Reason: %s",
                    currentUser.getEmail(), id, cleanReason);
        } else {
            description = String.format("Customer permanently deleted their own account. Reason: %s", cleanReason);
        }

        // Capture variables to ensure they aren't lost post-delete
        Long currentUserId = currentUser.getId();
        String currentUserEmail = currentUser.getEmail();
        String currentUserRole = currentUser.getRoles().iterator().next().name();
    ...
    public ResponseEntity<String> deleteEmployee(@PathVariable("id") Long id,@RequestBody @Valid String reason,Authentication authentication,
        ...
        // Capture the required primitives/strings before the DB state changes
        Long adminId = admin.getId();
        String adminRole = admin.getRoles().isEmpty() ? "ADMIN" : admin.getRoles().iterator().next().name();
        String cleanReason = reason.replace("\"", "").trim();
        String description = String.format("Admin %s soft deleted employee account (Target ID: %d) (Reason: %s)", adminEmail, id, cleanReason);
    ...
    public ResponseEntity<String> deactivateOwnAccount(@RequestBody @Valid String reason,Authentication authentication, HttpServletRequest httpRequest) {
        ...
        // --- 1. FETCH USER DETAILS BEFORE DEACTIVATION ---
        User currentUser = userService.findByEmail(email);

        // Capture log parameters into local memory variables
        Long currentUserId = currentUser.getId();
        String currentUserEmail = currentUser.getEmail();
        String currentUserRole = currentUser.getRoles().isEmpty() ? "CUSTOMER" : currentUser.getRoles().iterator().next().name();
        String cleanReason = reason.replace("\"", "").trim();
        String description = String.format("Customer self-deactivated their own account (Reason: %s)", cleanReason);
